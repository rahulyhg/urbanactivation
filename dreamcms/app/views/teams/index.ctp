...
<div class="teams index">
    ...
    <div style="clear:both;display:block;height:30px">
    	<?php
			...
			if (!isset($_GET['group'])) {
				echo $this->Form->input('select_teams_category_id', array('type' => 'select','options' => $categoryOptions,'between' => ': ','onchange'=> $jsString));
			} else {
				$groupValue = $_GET['group'];
				echo $this->Form->input('select_teams_category_id', array('type' => 'select','options' => $categoryOptions,'between' => ': ','onchange'=> $jsString,'default' => $groupValue));
			}
		?>
    </div>
    ...
    <div id="records">
        ...
        <?php echo $ajax->sortable('teams', array('url'=>'order', 'before'=>"Element.hide('order-status');", 'complete'=>"Element.show('order-status');")); ?>
		<div id="clear"></div><br />
            <div class="paging">
            <?php
