<div class="propertiesCategories index">
	<h1><?php __('property categories');?></h1>
	...
	<?php
	if($propertiesCategories){
		$i = 0;
		foreach ($propertiesCategories as $propertiesCategory):
			$class = null;
			if ($i++ % 2 == 0) {
				$class = ' class="altrow"';
			}
	?>
		<div id="record_wrap" <?php echo $class;?>>
            <div style="width:20px" id="record_row">
                <div id="record_detail"><?php echo $propertiesCategory['PropertiesCategory']['id']; ?>&nbsp;</div>
            </div>
            <div style="width:80%" id="record_row">
                <div id="record_detail"><?php echo $propertiesCategory['PropertiesCategory']['category']; ?>&nbsp;</div>
            </div>
            <div style="width:50px" id="record_row">
                <div id="record_option" class="imgEdit"><?php echo $this->Html->link($html->image("edit.gif",array('id'=>'edit','alt'=>'edit')), array('action' => 'edit', $propertiesCategory['PropertiesCategory']['id']), array('escape' => false,'id'=>'record','title'=>'edit')); ?></div>
            </div>
            <div style="width:50px" id="record_row">
            <?php 	
					foreach ($hasNoPropertiesItems as $hasNoPages){
						if($hasNoPages['PropertiesCategory']['id'] === $propertiesCategory['PropertiesCategory']['id']){
			?>
                <div id="record_option" class="imgDelete"><?php echo $this->Html->link($html->image("delete.gif",array('id'=>'delete','alt'=>'delete')), array('action' => 'delete', $propertiesCategory['PropertiesCategory']['id']), array('escape' => false,'id'=>'record','title'=>'delete'), sprintf(__('Are you sure you want to delete # %s?', true), $propertiesCategory['PropertiesCategory']['id'])); ?></div>
            <?php		}
					}
			?>
            </div>
